...
int main() {
    ...
    // Решение с помощью ветвей и границ
    visited[0] = true;
    path[0] = 0;
    branchAndBound(0, 1, 0);

    std::cout << "Минимальная стоимость (ветви и границы): " << bestCostBranchAndBound << std::endl;
    std::cout << "Путь: ";
    for (int i = 0; i < N; ++i) {
        std::cout << bestPathBranchAndBound[i] + 1 << " ";
    }
    std::cout << 1 << std::endl;

    // Проверка решения при помощи генератора перестановок
